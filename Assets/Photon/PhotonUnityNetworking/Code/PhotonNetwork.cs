...
namespace Photon.Pun
{
    ...
    public static partial class PhotonNetwork
    {
        ...
        private static GameObject NetworkInstantiate(Pun.InstantiateParameters parameters, bool roomObject = false, bool instantiateEvent = false)
        {
            ...
            if (!PrefabsWithoutMagicCallback.Contains(parameters.prefabName))
            {
                ...
                if (list.Length > 0)
                {
                    ...
                    foreach (IPunInstantiateMagicCallback callbackComponent in list)
                    {
                        ...
                        //AvatarState state = (AvatarState)pmi.photonView.InstantiationData[0];
                        //Debug.Log("헤어코드 : " + state.hairCode);
                    }
                }
                else
                {
            ...
            //AvatarState state = JsonUtility.FromJson<AvatarState>(parameters.data[0].ToString());
            //go.GetComponent<CharctorMeshAndMaterialController>().CharacterSetting(state);
